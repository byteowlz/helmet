...
use helmet_core::{AppConfig, AppPaths};
...
#[derive(Debug, Parser)]
#[command(author, version, about = "MCP server for helmet")]
struct Cli {
    #[command(flatten)]
    common: CommonOpts,
}
    ...
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            server_info: Implementation {
                name: "helmet-mcp".to_string(),
                version: env!("CARGO_PKG_VERSION").to_string(),
            },
            instructions: Some("MCP server for helmet prompt injection detection".to_string()),
        }
    }
}
